...
Note = DS.Model.extend
  ...
  pitch: Em.computed 'pitchNumber', ->
    Pitch.all.findBy 'number', @get('pitchNumber')

  setPitchNumber: (->
    @set 'pitchNumber', Pitch.numberAtScale(@get('position.y'))
  ).observes('position.y')
  ...
  moveUp: (num) ->
    @set 'pitchNumber', Pitch.highest @get('pitchNumber') + num

  moveDown: (num) ->
    @set 'pitchNumber', Pitch.lowest @get('pitchNumber') - num

  changeDuration: (editResolution) ->
    if @get('duration') + editResolution > 0
      @incrementProperty('duration', editResolution)

Note.TICKS_PER_BEAT = TICKS_PER_BEAT
